...
if($_POST!=""){
	if($_POST["student_id"]=="" || $_POST["student_gender"]=="" || $_POST["student_tel"]=="" || $_POST["student_name"]==""){
		...
		$sqlbuy="INSERT INTO `buy`(`student_name`, `student_id`, `student_sex`, `student_tel`, `total_price`, `bonus_num`, `team`) VALUES ('".$_POST["student_name"]."',".$_POST["student_id"].",".$_POST["student_gender"].",".$_POST["student_tel"].",".$_POST["endprice"].",'".$bonus_id."',4)";
		mysql_query($sqlbuy);
		...
		foreach($_SESSION['product'] as list($p_id, $num, $addp_id)){
			...
			//echo $sqldetail."<br>";		
			//加購產品
			if(@$_POST["addproduct"]!=0){
				$addp_id=explode(",",$_POST["addproduct"]);
				foreach($addp_id as $myvalue => $value){
					$addprice="select pro_price from product where p_id=".$value;
					$resultadd=mysql_fetch_array(mysql_query($addprice));
					$sqldetail="INSERT INTO `buy_detail`(`buy_id`, `prduct_id`, `product_num`, `subtotal`) VALUES (".$buy_id.",".$value.",1,".$resultadd["pro_price"].")";
					mysql_query($sqldetail);
					//echo $sqldetail;
				}
			}
